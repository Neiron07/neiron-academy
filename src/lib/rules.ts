...
export const COIN_RULES = {
  ...
  module_done:       { coins: 200, xp: 100 },
  course_done:       { coins: 100, xp: 200 },
} as const;

/**
 * Витрина «за что дают нейроны» — то, что реально видят ученик и препод
 * (страницы-гайды в обоих кабинетах). Не всё здесь автоматизировано:
 * посещаемость/домашка/стрик/модуль начисляются кодом выше (COIN_RULES),
 * а разовые достижения (проект, конкурс, демо-день, привёл друга) — это
 * решение админа/учителя через ручное начисление коинов, здесь просто
 * ориентир по сумме, чтобы начисляли одинаково по всей школе.
 */
export const COIN_GUIDE = [
  { label: 'Присутствие на уроке', coins: 10 },
  { label: 'Домашка сдана вовремя', coins: 15 },
  { label: 'Активность на уроке, помощь другому', coins: 10 },
  { label: 'Завершённый проект', coins: 60 },
  { label: '4 урока подряд без пропусков', coins: 30 },
  { label: 'Завершённый модуль (3 месяца)', coins: 200 },
  { label: 'Победа во внутреннем конкурсе', coins: 150 },
  { label: 'Выступление на демо-дне', coins: 100 },
  { label: 'Привёл друга, который оплатил', coins: 500 },
] as const;
...
export const MANUAL_REASONS = [
  'Активность на уроке, помощь другому',
