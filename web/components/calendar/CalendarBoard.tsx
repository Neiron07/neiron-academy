...
/**
 * Календарь персонала: общий и для админа, и для маркетолога — единственное
 * отличие в правах — отмена урока остаётся только у админа (canCancelLessons).
 */
                ...
                {(byDay.get(day) ?? []).map((entry) =>
                  entry.type === 'lesson' ? (
                    <LessonEntry
                      key={entry.data.id}
                      lesson={entry.data}
                      canCancel={canCancelLessons}
                      onCancel={() => setCancelLesson(entry.data)}
                    />
                  ) : (
                    <EventEntry
                      key={entry.data.id}
...
function LessonEntry({ lesson, canCancel, onCancel }: { lesson: CalendarLesson; canCancel: boolean; onCancel: () => void }) {
  ...
  return (
    <div className={`group relative rounded-xl border p-2 text-xs ${cancelled ? 'border-muted opacity-60' : 'border-purple-mid'}`}>
      ...
      {canCancel && !cancelled && lesson.status === 'planned' && (
        <button
          onClick={onCancel}
